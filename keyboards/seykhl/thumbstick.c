...
void thumbstick_init(void) {
    ...
    thumbstick_state.config.mode           = THUMBSTICK_MODE_SCROLL;
...
int16_t thumbstick_get_mouse_speed(int16_t component) {
    uint16_t speed = THUMBSTICK_SPEED;// thumbstick_state.config.speed;
    uint16_t f_speed = THUMBSTICK_FINE_SPEED;//thumbstick_state.config.fineSpeed;
    #ifdef USEPOT
    uint16_t slider = (analogReadPin(SPEED_PIN) * 128) / 1024;
    speed = (THUMBSTICK_SPEED * slider) / (128);
    f_speed = (THUMBSTICK_FINE_SPEED * slider) / (128);
    thumbstick_state.config.speed = speed;
    thumbstick_state.config.fineSpeed = f_speed;
    uprintf("analog: %d, %d; speed (%d, %d);\n", analogReadPin(SPEED_PIN), slider, thumbstick_state.config.speed, thumbstick_state.config.fineSpeed);

    #endif
    int16_t  maxSpeed;
    uint16_t distance = abs(component);
    if (distance > thumbstick_state.config.fineZone) {
        maxSpeed = thumbstick_state.config.speed;
    } else if (distance > THUMBSTICK_DEAD_ZONE) {
        maxSpeed = thumbstick_state.config.fineSpeed;
    } else {
        return 0;
    }
    ...
    switch (thumbstick_state.config.mode) {
        case THUMBSTICK_MODE_MOUSE:
            ...
            if (timer_elapsed(thumbstickLogTimer) > 100) {
                ...
                // uprintf("movement (%d, %d); speed (%d, %d);\n", report.x, report.y, thumbstick_state.config.speed, thumbstick_state.config.fineSpeed);
            }
#endif
            break;
        case THUMBSTICK_MODE_ARROWS:
